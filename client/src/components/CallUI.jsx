...
export function CallUI({
  ...
  // ALWAYS attach remoteStream to audio — this is the #1 priority
  useEffect(() => {
    ...
    el.play().catch(() => {});
    // Retry play every 500ms in case of autoplay policy
    const t = setInterval(() => {
  ...
  // Attach to video elements when available
  useEffect(() => {
      ...
      <div className={`call-overlay ${isVideo && !isRinging ? "call-video" : ""}`}>
        ...
        <div className={`call-controls ${isVideo && !isRinging ? "call-controls-video" : ""}`}>
          ...
          {isRinging && !call.isCaller && (
            <button className="call-btn call-answer" onClick={() => onAnswer(call)}>
