...
const SQLITE_SCHEMA = `
  ...
  CREATE INDEX IF NOT EXISTS idx_messages_thread ON messages(from_id, to_id, created_at);
  CREATE INDEX IF NOT EXISTS idx_messages_unread ON messages(to_id, read);
...
export async function initDb() {
  if (isSupabase) {
    ...
    const pgMigrations = [
      ...
      `CREATE UNIQUE INDEX IF NOT EXISTS idx_report_once ON reports(user_id, post_id)`,
      `CREATE TABLE IF NOT EXISTS messages (id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, from_id BIGINT NOT NULL REFERENCES users(id) ON DELETE CASCADE, to_id BIGINT NOT NULL REFERENCES users(id) ON DELETE CASCADE, body TEXT NOT NULL, kind TEXT NOT NULL DEFAULT 'text', read INTEGER NOT NULL DEFAULT 0, created_at TIMESTAMPTZ NOT NULL DEFAULT now())`,
      `CREATE INDEX IF NOT EXISTS idx_messages_thread ON messages(from_id, to_id, created_at)`,
      `CREATE INDEX IF NOT EXISTS idx_messages_unread ON messages(to_id, read)`,
      `ALTER TABLE messages ADD COLUMN IF NOT EXISTS kind TEXT NOT NULL DEFAULT 'text'`,
      `ALTER TABLE calls ADD COLUMN IF NOT EXISTS video BOOLEAN DEFAULT FALSE`,
      `CREATE TABLE IF NOT EXISTS push_subscriptions (id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, user_id BIGINT NOT NULL REFERENCES users(id) ON DELETE CASCADE, endpoint TEXT NOT NULL UNIQUE, p256dh TEXT NOT NULL, auth TEXT NOT NULL, created_at TIMESTAMPTZ NOT NULL DEFAULT now())`,
    ...
    ensureCol("calls", "video", "INTEGER DEFAULT 0");
    ...
    ensureCol("messages", "kind", "kind TEXT NOT NULL DEFAULT 'text'");
  }
}
